...
def find_number(letter):
    letterNumber = re.search(u'<h1>(\\d+)\.', letter)
    return letterNumber.group(1)

def find_addressee(letter):
    letterAddressee = re.search(u'<h1>\\d+\.\\s(.+?)<', letter)
...
def find_date(letter):
    try:
        letterDate = re.search(u'subhead">(.+?)(\.|,)', letter)
...
def find_place(letter):
    try:
        letterPlace = re.search(u'subhead">.+?(\.|,)\\s(.+?)<', letter)
...
def find_page(mainHtml, number):
    try:
        letterPage = re.search(u'<td><a\\s+href=".+?">' + number + u'\.\\s+.+?</a>.+?(\\d+)<', mainHtml)
...
def find_note_link(letter):
    try:
        letterNoteLink = re.search (u'(02comm/.+?)">Примечания', letter)
